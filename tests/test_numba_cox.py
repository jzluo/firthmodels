...
class TestFirthCoxPH:
    ...
    def test_numba_polars_non_numeric_raises(self):
        ...
        with pytest.raises(ValueError, match="could not convert|dtype"):
            FirthCoxPH(backend="numba").fit(X, (event, time))

    def test_numba_polars_negative_time_raises(self):
    ...
    def test_numba_polars_no_events_raises(self):
        X = pl.DataFrame({"x1": [0, 1, 0], "x2": [0.1, 0.2, 0.3]})
    ...
    def test_numba_polars_lazyframe_raises(self, cox_separation_data):
        X, time, event = cox_separation_data
        ...
        with pytest.raises(ValueError):
            FirthCoxPH(backend="numba").fit(X_pl, (event_s, time_s))
